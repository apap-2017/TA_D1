...
public interface KurikulumMapper {
	...
	// fdl
	// lihat hasil cari kurikulum milih prodi tertentu dengan fakultas tertentu di universitas tertentu
	@Select("select distinct k.kode_kurikulum, k.nama_kurikulum from kurikulum k, fakultas f, prodi p"
			+ "where k.id_prodi=p.id_prodi and p.id_fakultas=f.id_fakultas and"
			+ "k.id_prodi=#{id_prodi} and k.id_fakultas=#{id_fakultas}")
